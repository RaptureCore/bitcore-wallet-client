...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testHardcodedKeys() {
    ...
    var xpriv = Mnemonic_[c.coin](words).toHDPrivateKey();
    ...
    var xpub = Bitcore_[c.coin].HDPublicKey.fromString('xpub6BosfCnifzxcFwrSzQiqu2DBVTshkCXacvNsWGYJVVhhawA7d4R5WSWGFNbi8Aw6ZRc1brxMyWMzG3DSSSSoekkudhUd9yLb6qx39T9nMdj');
  ...
  function testLiveKeys() {
    ...
    if (!xpriv) {
      xpriv = new Bitcore_[coin].HDPrivateKey(c.xPrivKey);
    }
    xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath());
    var xpub = new Bitcore_[coin].HDPublicKey(c.xPubKey);
  ...
  if (self.credentials) {
    ...
    if (useSession && self.session) {
      ...
      if (key) {
        ...
        reqSignature = API._signRequest(method, url, args, key, coin);
      }
...
API._buildSecret = function(walletId, walletPrivKey, coin, network) {
  ...
  var widBase58 = new Bitcore_[coin].encoding.Base58(widHex).toString();
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new Bitcore_[txp.coin].HDPrivateKey(derivedXPrivKey);
...
API.prototype._doJoinWallet = function(walletId, walletPrivKey, xPubKey, requestPubKey, copayerName, opts, cb) {
  ...
  args.copayerSignature = Utils.signMessage(hash, walletPrivKey,args.coin);
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  $.checkArgument(coin || _.includes(['btc', 'bch', 'rap'], coin));
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var coin = opts.coin || 'rap';  //RAP as default
  if (!_.includes(['btc', 'bch', 'rap'], coin)) return cb(new Error('Invalid coin'));
...
API.prototype.publishTxProposal = function(opts, cb) {
  ...
  var args = {
    proposalSignature: Utils.signMessage(hash, self.credentials.requestPrivKey,this.credentials.coin)
  };
...
API.prototype.getBalance = function(opts, cb) {
  ...
  if (opts.coin) {
    if (!_.includes(['btc', 'bch', 'rap'], opts.coin)) return cb(new Error('Invalid coin'));
