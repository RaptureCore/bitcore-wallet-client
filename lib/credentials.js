...
var Bitcore_ = {
  ...
  rap: require('bitcore-lib-rapture'),
};
var Mnemonic = require('bitcore-mnemonic');
var Mnemonic_ = {
  btc: Mnemonic,
  bch: Mnemonic,
  rap: require('bitcore-mnemonic-rapture'),
...
function _checkCoin(coin) {
  if (!_.includes(['btc', 'bch', 'rap'], coin)) throw new Error('Invalid coin');
};
...
Credentials.createWithMnemonic = function(coin, network, passphrase, language, account, opts) {
  ...
  var m = new Mnemonic_[coin](wordsForLang[language]);
  while (!Mnemonic_[coin].isValid(m.toString())) {
    m = new Mnemonic_[coin](wordsForLang[language])
  };
  var x = new Credentials();
...
Credentials.fromMnemonic = function(coin, network, words, passphrase, account, derivationStrategy, opts) {
  ...
  var m = new Mnemonic_[coin](words);
  var x = new Credentials();
  x.coin = coin;
  x.xPrivKey = m.toHDPrivateKey(passphrase, network).toString();
  x.mnemonic = words;
...
Credentials.fromExtendedPublicKey = function(coin, xPubKey, source, entropySourceHex, account, derivationStrategy, opts) {
  ...
  x.coin = coin;
  x.xPubKey = xPubKey;
  x.entropySource = Bitcore_[coin].crypto.Hash.sha256sha256(entropyBuffer).toString('hex');
  x.account = account;
  x.derivationStrategy = derivationStrategy;
...
Credentials.prototype._hashFromEntropy = function(prefix, length) {
  ...
  var b2 = Bitcore_[this.coin].crypto.Hash.sha256hmac(b, new Buffer(prefix));
...
Credentials.prototype._expand = function() {
  ...
  if (this.entropySourcePath) {
    ...
    this.entropySource = Bitcore_[this.coin].crypto.Hash.sha256sha256(seed).toString('hex');
  }

  if (this.entropySource) {
    // request keys from entropy (hw wallets)
    var seed = this._hashFromEntropy('reqPrivKey', 32);
    var privKey = new Bitcore_[this.coin].PrivateKey(seed.toString('hex'), network);
    ...
    this.entropySource = Bitcore_[this.coin].crypto.Hash.sha256(requestDerivation.privateKey.toBuffer()).toString('hex');
  }
...
Credentials.prototype.getDerivedXPrivKey = function(password) {
  ...
  var xPrivKey = new Bitcore_[this.coin].HDPrivateKey(this.getKeys(password).xPrivKey, this.network);
  var deriveFn = !!this.compliantDerivation ? _.bind(xPrivKey.deriveChild, xPrivKey) : _.bind(xPrivKey.deriveNonCompliantChild, xPrivKey);
...
Credentials.fromOldCopayWallet = function(w) {
  function walletPrivKeyFromOldCopayWallet(w) {
    ...
    var privKey = new Bitcore_[this.coin].PrivateKey.fromBuffer(Bitcore_[this.coin].crypto.Hash.sha256(seedBuf));
  ...
  var pkr = _.map(w.publicKeyRing.copayersExtPubKeys, function(xPubStr) {
    ...
    if (isMe) {
      ...
      requestDerivation = (new Bitcore_[this.coin].HDPrivateKey(credentials.xPrivKey))
        .deriveChild(path).hdPublicKey;
    } else {
      // this 
      var path = Constants.PATHS.REQUEST_KEY_AUTH;
      requestDerivation = (new Bitcore_[this.coin].HDPublicKey(xPubStr)).deriveChild(path);
    }

    // Grab Copayer Name
    var hd = new Bitcore_[this.coin].HDPublicKey(xPubStr).deriveChild('m/2147483646/0/0');
